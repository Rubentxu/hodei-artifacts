...
export function useCreateRepository() {
  ...
  return useMutation<Repository, Error, CreateRepositoryRequest>({
    ...
    onSuccess: newRepository => {
      ...
      queryClient.setQueryData(QUERY_KEYS.REPOSITORY(newRepository.id), {
        data: newRepository,
      });

      showSuccess(
        'Repositorio creado',
        `El repositorio "${newRepository.name}" se ha creado exitosamente`
      );
    },
    onError: error => {
...
export function useUpdateRepository() {
  ...
  >({
    mutationFn: ({ id, data }) =>
      repositoryService.actualizarRepositorio(id, data),
    onSuccess: (updatedRepository, variables) => {
      // Actualizar el repositorio en el caché
      queryClient.setQueryData(QUERY_KEYS.REPOSITORY(variables.id), {
      ...
      showSuccess(
        ...
        `El repositorio "${updatedRepository.name}" se ha actualizado exitosamente`
      );
    },
  ...
  return useMutation<void, Error, string>({
    ...
    onSuccess: (_, deletedId) => {
      ...
      showSuccess(
        ...
        'El repositorio se ha eliminado exitosamente'
      );
    },
    onError: error => {

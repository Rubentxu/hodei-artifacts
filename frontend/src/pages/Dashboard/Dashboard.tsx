...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Cargar datos en paralelo usando servicios mock mejorados
      ...
      const total = [...popularData, ...recentData].reduce((sum, pkg) => sum + pkg.downloadCount, 0);
                ...
                {repositories.map((repo) => (
                  <div key={repo.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer">
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge variant="secondary">{repo.createdAt.split('T')[0]}</Badge>
            ...
            <div className="p-6">
              <div className="space-y-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-gray-900">
                      <span className="font-medium">react</span> package uploaded to npm-public
                    </p>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-gray-900">
                      New repository <span className="font-medium">spring-boot-configs</span> created
                    </p>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-gray-900">
                      <span className="font-medium">junit</span> downloaded 234 times
                    </p>
                ...
                {popularPackages.map((pkg) => (
                  <div key={pkg.id} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <Badge variant="secondary">{pkg.downloadCount.toLocaleString()}</Badge>
                    </div>
                  </div>
                ))}
              </div>
                ...
                {recentPackages.map((pkg) => (
                  <div key={pkg.id} className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <h4 className="font-medium text-gray-900">{pkg.name}</h4>
                        <p className="text-xs text-gray-600">{pkg.repositoryName}</p>
                      </div>
                    </div>

...
impl OrganizationBoundaryProvider for SurrealOrganizationBoundaryProvider {
    ...
    async fn get_effective_scps_for(
        ...
        // Create use case
        let use_case =
            get_effective_scps_use_case(scp_repository, account_repository, ou_repository);
        ...
        for scp_hrn_string in result.effective_scps {
            let scp_hrn = Hrn::from_str(&scp_hrn_string).map_err(|e| {
                EvaluatePermissionsError::OrganizationBoundaryProvider(e.to_string())
            })?;

            // Find the actual SCP object
            let scp_repository = SurrealScpRepository::new(self.db.clone());
            let scp = scp_repository
                .find_by_hrn(&scp_hrn)
                .await
                .map_err(|e| EvaluatePermissionsError::OrganizationBoundaryProvider(e.to_string()))?
                .ok_or_else(|| {
                    EvaluatePermissionsError::OrganizationBoundaryProvider(format!(
                        "SCP not found: {}",
                        scp_hrn_string
                    ))
                })?;
            ...
            let policy = cedar_policy::Policy::from_str(&scp.policy_text).map_err(|e| {
                EvaluatePermissionsError::OrganizationBoundaryProvider(format!(
                    "Failed to parse SCP policy: {}",
                    e
                ))
            })?;

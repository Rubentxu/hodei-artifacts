...
use cedar_policy::{RestrictedExpression, EntityUid, Entity, Schema, SchemaFragment};
...
impl TestUser {
    fn new(id: String, name: String, email: String, groups: Vec<String>, tags: Vec<String>) -> Self {
        Self { id, name, email, groups, tags }
    }
    ...
    fn attributes(&self) -> HashMap<String, RestrictedExpression> {
        ...
        let group_expressions: Vec<RestrictedExpression> = self.groups
            .iter()
            .map(|group| RestrictedExpression::new_string(group.clone()))
            .collect();
        attrs.insert("groups".to_string(), RestrictedExpression::new_set(group_expressions));
        
        let tag_expressions: Vec<RestrictedExpression> = self.tags
            .iter()
            .map(|tag| RestrictedExpression::new_string(tag.clone()))
            .collect();
        attrs.insert("tags".to_string(), RestrictedExpression::new_set(tag_expressions));

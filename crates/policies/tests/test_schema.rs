...
use cedar_policy::{Schema, SchemaFragment, RestrictedExpression, EntityUid, Entity, Entities, Context, Request, PolicySet, SchemaError};
...
fn test_complete_schema_build() {
    ...
    assert!(result.is_ok(), "Failed to create schema fragment: {:?}", result.err());
    ...
    assert!(schema_result.is_ok(), "Failed to build complete schema: {:?}", schema_result.err());
    ...
    let user = TestUser::new("test_user".to_string(), "Test User".to_string(), "test@example.com".to_string());
    let parents: HashSet<_> = user.parents().into_iter().collect();
    let entity = Entity::new(user.euid(), user.attributes(), parents);
    assert!(entity.is_ok(), "Failed to create entity: {:?}", entity.err());
}
...
fn test_policy_evaluation_with_restricted_expressions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let alice_attrs: HashMap<String, RestrictedExpression> = [
        ("name".to_string(), RestrictedExpression::new_string("Alice".to_string())),
        ("email".to_string(), RestrictedExpression::new_string("alice@example.com".to_string())),
    ].into_iter().collect();
    ...
    let bob_attrs: HashMap<String, RestrictedExpression> = [
        ("name".to_string(), RestrictedExpression::new_string("Bob".to_string())),
        ("email".to_string(), RestrictedExpression::new_string("bob@example.com".to_string())),
    ].into_iter().collect();

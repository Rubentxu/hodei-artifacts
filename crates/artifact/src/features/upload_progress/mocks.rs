...
use super::dto::{ReceivedChunkInfo, UpdateProgressCommand, UploadProgress};
use super::ports::{
...
impl ChunkedUploadStorage for MockChunkedUploadStorage {
    ...
    async fn get_received_chunks_info(
        ...
        let chunk_info = chunk_numbers
            .into_iter()
            .map(|num| ReceivedChunkInfo {
                chunk_number: num,
                size: 1024, // Mock size
            })
            .collect();
    ...
    async fn assemble_chunks(
        ...
        _total_chunks: usize,
        _file_name: &str,
    ) -> Result<(PathBuf, String), ProgressError> {

...
use super::dto::{
    ReceivedChunkInfo, ReceivedChunksResponse, UpdateProgressCommand, UploadProgress, UploadStatus,
};
use super::ports::{
    ChunkedUploadStorage, ProgressEventPublisher, ProgressResult, ProgressStorage, RealtimeNotifier,
};
...
impl UploadProgressUseCase {
    ...
    pub async fn update_progress(
        ...
        if let Err(e) = self
            .event_publisher
            .publish_progress_update(&updated_progress)
            .await
        {
        ...
        if let Err(e) = self
            .realtime_notifier
            .notify_progress_update(&updated_progress)
            .await
        {
    ...
    pub async fn mark_completed(&self, upload_id: &str) -> ProgressResult<UploadProgress> {
        ...
        // Obtener el progreso actual para conocer el total_bytes
        let current = self.storage.get_progress(upload_id).await?;
        // Establecer bytes_transferred = total_bytes para alcanzar 100%
        let command = UpdateProgressCommand {
            upload_id: upload_id.to_string(),
            bytes_transferred: current.total_bytes,
        ...
        if let Err(e) = self
            .event_publisher
            .publish_upload_completed(upload_id)
            .await
        {
    ...
    pub async fn mark_failed(
        ...
        // Mantener los valores actuales de progreso/total para no resetear a 0
        let current = self.storage.get_progress(upload_id).await?;
        let command = UpdateProgressCommand {
            upload_id: upload_id.to_string(),
        ...
        if let Err(e) = self
            .event_publisher
            .publish_upload_failed(upload_id, error_message)
            .await
        {
    ...
    pub async fn list_sessions(&self) -> ProgressResult<Vec<UploadProgress>> {
        ...
        info!(
            ...
            "Retrieved active upload sessions"
        );
        Ok(sessions)
    }
    ...
    pub async fn get_received_chunks(
        &self,
        upload_id: &str,
    ) -> ProgressResult<ReceivedChunksResponse> {
        info!(upload_id = %upload_id, "Getting received chunks for upload");

        // Verificar que tenemos acceso al chunked storage
        let chunked_storage = self.chunked_storage.as_ref().ok_or_else(|| {
            ProgressError::StorageError("Chunked storage not available".to_string())
        })?;
        ...
        let chunk_numbers = chunked_storage

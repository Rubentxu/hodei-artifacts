...
impl ContentTypeDetector for MockDetector {
    ...
    async fn validate_consistency(&self, detected: &str, provided: Option<&str>) -> Result<ContentTypeDetectionResult, ContentTypeDetectionError> {
        Ok(ContentTypeDetectionResult {
...
async fn test_detect_content_type_success() {
    ...
    let result = use_case.detect_content_type(data, Some("file.pdf"), Some("application/pdf")).await;
    assert!(result.is_ok());

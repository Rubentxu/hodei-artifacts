...
async fn test_execute_batch_success() {
    ...
    let use_case =
        BatchUploadUseCase::new(artifact_processor.clone(), transaction_manager.clone(), 30);
...
async fn test_execute_batch_partial_failure() {
    ...
    let commands = vec![BatchUploadArtifactCommand {
        coordinates: shared::models::PackageCoordinates {
            namespace: Some("test-org".to_string()),
            name: "test-package".to_string(),
            version: "1.0.0".to_string(),
...
async fn test_execute_batch_with_transaction_success() {
    ...
    assert_eq!(
        *transaction_manager.transactions_committed.lock().unwrap(),
        1
    );
    assert_eq!(
        *transaction_manager.transactions_rolled_back.lock().unwrap(),
        0
    );
}
...
async fn test_execute_batch_invalid_input() {
    ...
    assert!(matches!(
        result.unwrap_err(),
        BatchUploadError::InvalidRequest(_)
    ));
}
...
async fn test_should_rollback_on_critical_failure() {
    ...
    impl BatchArtifactProcessor for CriticalFailureProcessor {
        async fn process_artifact(
            ...
            Err(BatchUploadError::RepositoryError(
...
async fn test_should_not_rollback_on_non_critical_failure() {
    ...
    impl BatchArtifactProcessor for NonCriticalFailureProcessor {
        async fn process_artifact(
            ...
            Err(BatchUploadError::ValidationFailed(

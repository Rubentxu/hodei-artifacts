...
mod tests {
    ...
    fn test_valid_semver_versions() {
        ...
        for version in valid_versions {
            assert!(validator.validate_version(version).is_ok(), 
    ...
    fn test_invalid_semver_versions() {
        ...
        let invalid_versions = vec![
            ...
            "1.0.0.0",
            "a.b.c",
            "1.0.0-", // Tag pre-release vacío
            "1.0.0-!", // Carácter no válido en tag pre-release
        ];
        
        for version in invalid_versions {
            assert!(validator.validate_version(version).is_err(), 
    ...
    fn test_snapshot_versions() {
        ...
        for version in snapshot_versions {
            let parsed = validator.parse_version(version).expect(
                &format!("Version {} should be parseable", version)
            );
            assert!(parsed.is_snapshot, 
                    "Version {} should be identified as SNAPSHOT", version);
        }
    }
    ...
    fn test_strict_semver_enforcement() {
        ...
        for version in strict_versions {
            assert!(validator.validate_version(version).is_ok(), 
                    "Version {} should be valid under strict SemVer", version);
        }
    }
    ...
    fn test_prerelease_tag_restrictions() {
        ...
        let allowed_versions = vec![
            "1.0.0-alpha",
            "1.0.0-beta.1",
            "2.0.0-alpha.beta.gamma",
        ];
        
        for version in allowed_versions {
            assert!(validator.validate_version(version).is_ok(), 
                    "Version {} should be allowed", version);
        }
        
        // Versiones con tags no permitidos
        let forbidden_versions = vec![
            "1.0.0-rc.1",
            "1.0.0-nightly",
        ];
        ...
        for version in forbidden_versions {
            match validator.validate_version(version) {
                ...
                _ => {
                    panic!("Version {} should be rejected for forbidden prerelease tag", version);
                }
            }
        }
    }
    ...
    fn test_build_metadata_rejection() {
        ...
        let versions_without_build = vec![
            "1.0.0",
            "1.0.0-alpha.1",
        ];

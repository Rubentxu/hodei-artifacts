...
impl VersioningUseCase {
    ...
    pub async fn execute(
    ...
    ) -> Result<VersionValidationResult, VersioningError> {
        info!(
            "Validating version {} for package {}",
            command.version, command.package_hrn
        );
        ...
        if let Err(e) = self
            .version_validator
            .validate_version(&parsed_version, &config)
        {
        ...
        if self
            .repository
            .version_exists(&command.package_hrn, &command.version)
            .await?
        {
            errors.push(format!(
        ...
        if parsed_version.is_snapshot && config.allow_only_one_snapshot_per_major_minor {
            if self
                .repository
                .snapshot_exists_for_major_minor(
                    &command.package_hrn,
            ...
            {
                errors.push(format!(
                    "Snapshot version already exists for {}.{}",
        ...
        if is_valid {
            self.publish_validation_success_event(&command.package_hrn, &command.version)
                .await;
        } else {
            self.publish_validation_failed_event(
                &command.package_hrn,
        ...
        info!(
            "Version validation completed for {}: {}",
            command.package_hrn,
    ...
    pub async fn get_versioning_config(
        ...
        self.repository.get_versioning_config(repository_hrn).await
    }
    ...
    pub async fn update_versioning_config(
        ...
        info!(
            "Updating versioning config for repository {}",
            repository_hrn
        );
        self.repository
    ...
    pub async fn get_existing_versions(
        ...
        self.repository.get_existing_versions(package_hrn).await
    }

    /// Publish validation success event
    async fn publish_validation_success_event(
        &self,
        package_hrn: &shared::hrn::Hrn,
        version: &str,
    ) {
    ...
    /// Publish validation failed event
    async fn publish_validation_failed_event(
        &self,
        package_hrn: &shared::hrn::Hrn,
        version: &str,
        errors: Vec<String>,
        ...
        if let Err(e) = self
            .event_publisher
            .publish_version_validation_failed(event)
            .await
        {

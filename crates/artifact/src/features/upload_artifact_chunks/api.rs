...
impl ChunkedUploadEndpoint {
    ...
    pub async fn upload_chunk(
        ...
        info!("Uploading chunk {} for session {}", chunk_number, session_id);
    ...
    pub async fn abort_upload(
        ...
        let command = AbortChunkedUploadCommand {
            session_id,
            reason,
        };
...
impl IntoResponse for ChunkedUploadError {
    fn into_response(self) -> axum::response::Response {
        let (status, error_message) = match self {
            ...
            ChunkedUploadError::ChunkSizeMismatch { .. } => (StatusCode::BAD_REQUEST, self.to_string()),
            ChunkedUploadError::ChunkChecksumFailed(msg) => (StatusCode::BAD_REQUEST, msg),
            ChunkedUploadError::FinalChecksumFailed { .. } => (StatusCode::BAD_REQUEST, self.to_string()),

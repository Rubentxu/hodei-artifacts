use super::{
    dto::{
        ...
        UploadChunkResult, UploadSession, UploadSessionStatus, UploadStats,
    },
    error::ChunkedUploadError,
    ports::{
        ChunkStorage, ChunkedUploadEventPublisher, ChunkedUploadProgress,
        ChunkedUploadProgressTracker, ChunkedUploadSessionRepository,
    },
};
use std::sync::Arc;
...
impl ChunkedUploadUseCase {
    ...
    pub async fn initiate_upload(
        ...
        self
            .progress_tracker
            .init_session_progress(&session_id, session.chunk_count, session.total_size)
            .await?;
    ...
    pub async fn upload_chunk(
        ...
        info!(
            ...
            command.chunk_number, command.session_id
        );
        ...
        if session.status != UploadSessionStatus::InProgress {
            return Err(ChunkedUploadError::SessionAlreadyCompleted(
                command.session_id,
            ));
        }
        ...
        // Validate chunk number (accept 1-based indices as per API/tests)
        if command.chunk_number == 0 || command.chunk_number > session.chunk_count {
            return Err(ChunkedUploadError::InvalidChunkNumber(format!(
                "Chunk number {} is out of range (1-{})",
                command.chunk_number, session.chunk_count
            )));
        }
        ...
        info!(
            "Chunk {} uploaded successfully for session {}",
            command.chunk_number, command.session_id
        );
    ...
    pub async fn complete_upload(
        &self,
        command: CompleteChunkedUploadCommand,
    ) -> Result<CompleteChunkedUploadResult, ChunkedUploadError> {
        info!(
            "Completing chunked upload for session {}",
            command.session_id
        );
        ...
        let progress = self
            .progress_tracker
            .get_session_progress(&command.session_id)
            .await?;

        // Validate that all chunks were uploaded
        if progress.chunks_uploaded != session.chunk_count {
            return Err(ChunkedUploadError::ValidationError(format!(
                "Expected {} chunks, but only {} were uploaded",
                session.chunk_count, progress.chunks_uploaded
            )));
        }
        ...
        let storage_path = self
            ...
            .assemble_chunks(
                ...
                &format!(
                    "artifacts/{}/{}",
                    session.repository_hrn, session.package_hrn
                ),
            )
            .await?;
        ...
        self.progress_tracker
            .complete_session_progress(&command.session_id, &storage_path)
            .await?;
        ...
        let stats = UploadStats {
            ...
            upload_duration_ms: (OffsetDateTime::now_utc() - session.created_at)
                .whole_milliseconds() as u64,
            avg_speed_bps: if progress.bytes_uploaded > 0 {
                progress.bytes_uploaded as f64
                    / ((OffsetDateTime::now_utc() - session.created_at).whole_seconds() as f64)
            } else {
                0.0
            },
        };
        ...
        info!(
            "Chunked upload completed successfully for session {}",
            command.session_id
        );
    ...
    pub async fn abort_upload(
        &self,
        command: AbortChunkedUploadCommand,
        ...
        if let Err(e) = self
            .chunk_storage
            .delete_session_chunks(&command.session_id)
            .await
        {
        ...
        if let Err(e) = self
            .progress_tracker
            .reset_session_progress(&command.session_id)
            .await
        {

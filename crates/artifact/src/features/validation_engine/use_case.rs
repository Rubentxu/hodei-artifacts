...
impl ValidationEngineUseCase {
    ...
    pub async fn execute(&self, command: ValidateArtifactCommand) -> Result<ValidationResult, ValidationError> {
        ...
        let rules = self.rule_repository
        ...
        for rule in rules {
            info!("Executing validation rule: {} (priority: {})", rule.name, rule.priority);
        ...
        if !overall_valid {
            let event = ArtifactEvent::ValidationFailed {
        ...
        info!("Validation completed for {}: {}", command.package_version_hrn, 
    ...
    pub async fn get_active_rules(&self, artifact_type: &str) -> Result<Vec<ValidationRule>, ValidationError> {

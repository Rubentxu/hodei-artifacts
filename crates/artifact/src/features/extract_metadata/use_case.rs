...
use crate::domain::package_version::{ArtifactDependency, PackageCoordinates, PackageMetadata};
use std::sync::Arc;
...
impl ExtractMetadataUseCase {
    ...
    pub async fn execute(
    ...
    ) -> Result<MetadataExtractionResult, MetadataError> {
        info!(
            "Extracting metadata for package version: {}",
        ...
        let (metadata, dependencies) = match command.artifact_type.as_str() {
            ...
            _ => {
                ...
                return Err(MetadataError::UnsupportedArtifactType(
        ...
        info!(
            "Successfully extracted and published metadata for package: {}",
            command.package_version_hrn
        );
    ...
    fn convert_maven_metadata(
        &self,
        parsed_metadata: super::dto::ParsedMavenMetadata,
        maven_deps: Vec<super::dto::MavenDependency>,
    ) -> Result<(PackageMetadata, Vec<ArtifactDependency>), MetadataError> {
    ...
    fn convert_npm_metadata(
        &self,
        parsed_metadata: super::dto::ParsedNpmMetadata,
        npm_deps: Vec<super::dto::NpmDependency>,
    ) -> Result<(PackageMetadata, Vec<ArtifactDependency>), MetadataError> {
        let metadata = PackageMetadata {
            description: parsed_metadata.description,
            licenses: parsed_metadata.licenses,
        ...
        let dependencies = npm_deps
            ...
            .map(|dep| ArtifactDependency {
                ...
                scope: if dep.is_dev_dependency {
                    "dev".to_string()
                } else {

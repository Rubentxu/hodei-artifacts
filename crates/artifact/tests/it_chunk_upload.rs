...
async fn test_chunked_upload_storage_happy_path() -> Result<(), UploadArtifactError> {
    ...
    // Guardar chunks
    storage.save_chunk(upload_id, 1, chunk1_data.clone()).await?;
    storage.save_chunk(upload_id, 2, chunk2_data.clone()).await?;
    storage.save_chunk(upload_id, 3, chunk3_data.clone()).await?;
    ...
    let (assembled_path, hash) = storage.assemble_chunks(upload_id, 3, "test-file.txt").await?;
...
async fn test_chunked_upload_storage_chunk_overwrite() -> Result<(), UploadArtifactError> {
    ...
    // Guardar un chunk
    storage.save_chunk(upload_id, 1, chunk_data_v1.clone()).await?;
    
    // Guardar el mismo chunk con nuevos datos (sobreescribir)
    storage.save_chunk(upload_id, 1, chunk_data_v2.clone()).await?;

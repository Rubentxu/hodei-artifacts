...
async fn test_detect_policy_conflicts_integration_no_conflicts() -> Result<(), IamError> {
    ...
    println!("Conflict detection completed: {}", response.get_conflict_summary());
...
async fn test_detect_policy_conflicts_integration_with_potential_conflicts() -> Result<(), IamError> {
    ...
    let policies = vec![
        ...
        ).with_name("Permit Policy".to_string()).with_priority(1),
        PolicyForAnalysis::new(
            "forbid_policy".to_string(),
            "forbid(principal, action, resource);".to_string()
        ).with_name("Forbid Policy".to_string()).with_priority(2),
    ];
    ...
    if response.has_conflicts {
        ...
        for conflict in &response.conflict_analysis.conflicts {
            println!("  - Type: {:?}, Severity: {:?}", conflict.conflict_type, conflict.severity);
...
async fn test_detect_policy_conflicts_integration_redundant_policies() -> Result<(), IamError> {
    ...
    println!("  Total redundancies: {}", response.conflict_analysis.summary.total_redundancies);
...
async fn test_detect_policy_conflicts_integration_performance_metrics() -> Result<(), IamError> {
    ...
    println!("  Conflict detection: {}ms", response.metrics.conflict_detection_ms);
    println!("  Redundancy analysis: {}ms", response.metrics.redundancy_analysis_ms);
    println!("  Reachability analysis: {}ms", response.metrics.reachability_analysis_ms);
    println!("  Combinations analyzed: {}", response.metrics.combinations_analyzed);
...
async fn test_detect_policy_conflicts_integration_error_handling() -> Result<(), IamError> {
    ...
    let result = container.conflict_detection_service().detect_conflicts(request).await;
...
async fn test_detect_policy_conflicts_integration_large_policy_set() -> Result<(), IamError> {
    ...
    println!("  Policies analyzed: {}", response.conflict_analysis.summary.total_policies);
    println!("  Conflicts found: {}", response.conflict_analysis.summary.total_conflicts);
    println!("  Redundancies found: {}", response.conflict_analysis.summary.total_redundancies);
...
async fn test_detect_policy_conflicts_integration_with_context() -> Result<(), IamError> {
    ...
    println!("Context-aware analysis completed: {}", response.get_conflict_summary());

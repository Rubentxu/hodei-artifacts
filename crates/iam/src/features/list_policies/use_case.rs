...
impl ListPoliciesUseCase {
    ...
    pub async fn execute(&self, query: ListPoliciesQuery) -> Result<ListPoliciesResponse, ListPoliciesError> {
        // 1. Validate query
        query.validate()
            .map_err(|e| ListPoliciesError::InvalidInput(format!("Query validation failed: {}", e)))?;
        ...
        let policies = self.lister.list(filter).await
            .map_err(|e| ListPoliciesError::RepositoryError(format!("Failed to list policies: {}", e)))?;

...
impl From<ValidatePolicyError> for crate::infrastructure::errors::IamError {
    fn from(error: ValidatePolicyError) -> Self {
        match error {
            ...
            ValidatePolicyError::ValidationFailed { errors } => Self::PolicyValidationFailed { errors },
...
mod tests {
    ...
    fn test_validate_policy_error_display() {
        ...
        assert_eq!(error.to_string(), "Invalid policy syntax: Test syntax error");
    ...
    fn test_validate_policy_error_convenience_methods() {
        ...
        assert!(matches!(error, ValidatePolicyError::SchemaValidationFailed(_)));
    }
}
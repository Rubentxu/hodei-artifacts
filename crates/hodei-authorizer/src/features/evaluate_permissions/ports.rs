...
pub trait AuthorizationCache: Send + Sync {
    ...
    async fn put(&self, cache_key: &str, response: &AuthorizationResponse, ttl: std::time::Duration) -> EvaluatePermissionsResult<()>;
...
pub trait AuthorizationLogger: Send + Sync {
    ...
    async fn log_error(&self, request: &AuthorizationRequest, error: &super::error::EvaluatePermissionsError) -> EvaluatePermissionsResult<()>;
}
...
pub trait AuthorizationMetrics: Send + Sync {
    async fn record_decision(&self, decision: &super::dto::AuthorizationDecision, evaluation_time_ms: u64) -> EvaluatePermissionsResult<()>;
...
/// Trait for resolving entities from HRNs
#[async_trait]
...
#[async_trait]
impl<T: EntityResolver> EntityResolver for Arc<T> {
    async fn resolve_entity(&self, hrn: &Hrn) -> EvaluatePermissionsResult<Entity> {
        (**self).resolve_entity(hrn).await
    }
}

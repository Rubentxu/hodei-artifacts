...
#[tokio::test]
async fn test_allow_when_iam_allows_and_scp_allows() {
    // Arrange
...
#[tokio::test]
async fn test_deny_when_iam_denies() {
    // Arrange
    let iam_provider = MockIamPolicyProvider::new()
        .with_policy(
            Hrn::new("user", "test-user"),
...
#[tokio::test]
async fn test_deny_when_no_policies_allow() {
    // Arrange
    let iam_provider = MockIamPolicyProvider::new();
    let org_provider = MockOrganizationBoundaryProvider::new();
...
#[tokio::test]
async fn test_multiple_scps_effective() {
    // Arrange
    let iam_provider = MockIamPolicyProvider::new()
        .with_policy(
            Hrn::new("user", "test-user"),
    ...
    let org_provider = MockOrganizationBoundaryProvider::new()
        ...
        .with_scp(
            ...
            ServiceControlPolicy::new(
                Hrn::new("scp", "deny-scp"),
                "DenySCP".to_string(),
                "forbid(principal, action::\"ec2:TerminateInstances\", resource);".to_string(),
            ),
        );
...
#[tokio::test]
async fn test_iam_provider_error() {
    // Arrange
    let iam_provider = MockIamPolicyProvider::new();
    // Simulate an error by implementing a special error provider
    struct ErrorIamProvider;
    
    #[async_trait]
    impl IamPolicyProvider for ErrorIamProvider {
        async fn get_identity_policies_for(&self, _principal_hrn: &Hrn) -> Result<PolicySet, AuthorizationError> {
            Err(AuthorizationError::IamPolicyProvider("Test error".to_string()))
        }
    }
    
    let org_provider = MockOrganizationBoundaryProvider::new();
    let policy_evaluator = PolicyEvaluator::new();
    let authorizer = AuthorizerService::new(ErrorIamProvider, org_provider, policy_evaluator);
    ...
    assert_eq!(format!("{}", error), "IAM policy provider error: Test error");
}